...
{
	char *text, *converted_text;
	int err;

	text = g_strchomp(g_strndup(tag, length));
	if (mpg123_cfg.convert_in)
	{
		xmms_converter_convert(text, &converted_text, mpg123_cfg.convert_in_from, mpg123_cfg.convert_in_to, &err);
		g_free(text);
		text = converted_text;
	}
...
static void get_entry_tag(GtkEntry * entry, char * tag, int length)
{
	...
	if (mpg123_cfg.convert_out)
	{
		gchar *text;

		xmms_converter_convert(gtk_entry_get_text(entry), &text, mpg123_cfg.convert_out_from, mpg123_cfg.convert_out_to, &err);
