...
static GtkWidget *title_convert_in, *title_convert_out;
static GtkWidget *title_convert_in_from_list, *title_convert_in_to_list;
static GtkWidget *title_convert_out_from_list, *title_convert_out_to_list;
static GtkWidget *title_convert_in_from, *title_convert_in_to, *title_convert_out_from, *title_convert_out_to;
...
static void mpg123_configurewin_ok(GtkWidget * widget, gpointer data)
{
	...
	mpg123_cfg.convert_in = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(title_convert_in));

	if (mpg123_cfg.convert_in_from) {
		g_free(mpg123_cfg.convert_in_from);
		mpg123_cfg.convert_in_from = NULL;
	}
	tmp = xmms_converter_get_name_by_title(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(title_convert_in_from_list)->entry)));
	if (strlen(tmp) > 0) {
		if (strcasecmp(tmp, "user") == 0) {
			tmp = gtk_entry_get_text(GTK_ENTRY(title_convert_in_from));
			if (strlen(tmp) > 0)
				mpg123_cfg.convert_in_from = g_strdup(tmp);
		} else {
			mpg123_cfg.convert_in_from = g_strdup(tmp);
		}
	}
	if (mpg123_cfg.convert_in_to)
		g_free(mpg123_cfg.convert_in_to);
	mpg123_cfg.convert_in_to = NULL;
	tmp = xmms_converter_get_name_by_title(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(title_convert_in_to_list)->entry)));
	if (strlen(tmp) > 0) {
		if (strcasecmp(tmp, "user") == 0) {
			tmp = gtk_entry_get_text(GTK_ENTRY(title_convert_in_to));
			if (strlen(tmp) > 0)
				mpg123_cfg.convert_in_to = g_strdup(tmp);
		} else {
			mpg123_cfg.convert_in_to = g_strdup(tmp);
		}
	}

	mpg123_cfg.convert_out = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(title_convert_out));

	if (mpg123_cfg.convert_out_from)
		g_free(mpg123_cfg.convert_out_from);
	mpg123_cfg.convert_out_from = NULL;
	tmp = xmms_converter_get_name_by_title(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(title_convert_out_from_list)->entry)));
	if (strlen(tmp) > 0) {
		if (strcasecmp(tmp, "user") == 0) {
			tmp = gtk_entry_get_text(GTK_ENTRY(title_convert_out_from));
			if (strlen(tmp) > 0)
				mpg123_cfg.convert_out_from = g_strdup(tmp);
		} else {
			mpg123_cfg.convert_out_from = g_strdup(tmp);
		}
	}
	if (mpg123_cfg.convert_out_to)
		g_free(mpg123_cfg.convert_out_to);
	mpg123_cfg.convert_out_to = NULL;
	tmp = xmms_converter_get_name_by_title(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(title_convert_out_to_list)->entry)));
	if (strlen(tmp) > 0) {
		if (strcasecmp(tmp, "user") == 0) {
			tmp = gtk_entry_get_text(GTK_ENTRY(title_convert_out_to));
			if (strlen(tmp) > 0)
				mpg123_cfg.convert_out_to = g_strdup(tmp);
		} else {
			mpg123_cfg.convert_out_to = g_strdup(tmp);
		}
	}
	...
	xmms_cfg_write_boolean(cfg, "MPG123", "convert_in", mpg123_cfg.convert_in);
	xmms_cfg_write_string(cfg, "MPG123", "convert_in_from", mpg123_cfg.convert_in_from);
	xmms_cfg_write_string(cfg, "MPG123", "convert_in_to", mpg123_cfg.convert_in_to);
	xmms_cfg_write_boolean(cfg, "MPG123", "convert_out", mpg123_cfg.convert_out);
	xmms_cfg_write_string(cfg, "MPG123", "convert_out_from", mpg123_cfg.convert_out_from);
	xmms_cfg_write_string(cfg, "MPG123", "convert_out_to", mpg123_cfg.convert_out_to);
...
static void title_convert_in_cb(GtkWidget * w, gpointer data)
{
	gboolean convert_in;
	convert_in = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(title_convert_in));
	gtk_widget_set_sensitive(title_convert_in_from, convert_in);
	gtk_widget_set_sensitive(title_convert_in_to, convert_in);
	gtk_widget_set_sensitive(title_convert_in_from_list, convert_in);
	gtk_widget_set_sensitive(title_convert_in_to_list, convert_in);
}

static void title_convert_out_cb(GtkWidget * w, gpointer data)
{
	gboolean convert_out;
	convert_out = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(title_convert_out));
	gtk_widget_set_sensitive(title_convert_out_from, convert_out);
	gtk_widget_set_sensitive(title_convert_out_to, convert_out);
	gtk_widget_set_sensitive(title_convert_out_from_list, convert_out);
	gtk_widget_set_sensitive(title_convert_out_to_list, convert_out);
}
...
void mpg123_configure(void)
{
	...
	GtkWidget *title_convert_in_hbox, *title_convert_out_hbox;
	GtkWidget *title_convert_in_from_to, *title_convert_out_from_to;
...
#ifdef USE_SIMD
{
	...
	/* convert in */
	title_convert_in_hbox = gtk_hbox_new(FALSE, 5);
	gtk_box_pack_start(GTK_BOX(title_id3_vbox), title_convert_in_hbox, FALSE, FALSE, 0);

	title_convert_in_from_list = gtk_combo_new();
	title_convert_in_from = gtk_entry_new();
	title_convert_in_to_list = gtk_combo_new();
	title_convert_in_to = gtk_entry_new();

	title_convert_in = gtk_check_button_new_with_label(_("Convert Input"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(title_convert_in),
				     mpg123_cfg.convert_in);
	gtk_signal_connect(GTK_OBJECT(title_convert_in), "clicked", title_convert_in_cb, NULL);
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in, FALSE, FALSE, 0);

	gtk_entry_set_editable(GTK_ENTRY(GTK_COMBO(title_convert_in_from_list)->entry), FALSE);
	gtk_combo_set_value_in_list(GTK_COMBO(title_convert_in_from_list), TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in_from_list, TRUE, TRUE, 0);

	gtk_widget_set_sensitive(title_convert_in_from, mpg123_cfg.convert_in);
	gtk_entry_set_text(GTK_ENTRY(title_convert_in_from), mpg123_cfg.convert_in_from);
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in_from, FALSE, FALSE, 0);

	title_convert_in_from_to = gtk_label_new(_("->"));
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in_from_to, FALSE, FALSE, 0);

	gtk_entry_set_editable(GTK_ENTRY(GTK_COMBO(title_convert_in_to_list)->entry), FALSE);
	gtk_combo_set_value_in_list(GTK_COMBO(title_convert_in_to_list), TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in_to_list, TRUE, TRUE, 0);

	gtk_widget_set_sensitive(title_convert_in_to, mpg123_cfg.convert_in);
	gtk_entry_set_text(GTK_ENTRY(title_convert_in_to), mpg123_cfg.convert_in_to);
	gtk_box_pack_start(GTK_BOX(title_convert_in_hbox), title_convert_in_to, FALSE, FALSE, 0);

	/* convert out */
	title_convert_out_hbox = gtk_hbox_new(FALSE, 5);
	gtk_box_pack_start(GTK_BOX(title_id3_vbox), title_convert_out_hbox, FALSE, FALSE, 0);

	title_convert_out_from_list = gtk_combo_new();
	title_convert_out_from = gtk_entry_new();
	title_convert_out_to_list = gtk_combo_new();
	title_convert_out_to = gtk_entry_new();

	title_convert_out = gtk_check_button_new_with_label(_("Convert Output"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(title_convert_out),
				     mpg123_cfg.convert_out);
	gtk_signal_connect(GTK_OBJECT(title_convert_out), "clicked", title_convert_out_cb, NULL);
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out, FALSE, FALSE, 0);

	gtk_entry_set_editable(GTK_ENTRY(GTK_COMBO(title_convert_out_from_list)->entry), FALSE);
	gtk_combo_set_value_in_list(GTK_COMBO(title_convert_out_from_list), TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out_from_list, TRUE, TRUE, 0);

	gtk_widget_set_sensitive(title_convert_out_from, mpg123_cfg.convert_out);
	gtk_entry_set_text(GTK_ENTRY(title_convert_out_from), mpg123_cfg.convert_out_from);
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out_from, FALSE, FALSE, 0);

	title_convert_out_from_to = gtk_label_new(_("->"));
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out_from_to, FALSE, FALSE, 0);

	gtk_entry_set_editable(GTK_ENTRY(GTK_COMBO(title_convert_out_to_list)->entry), FALSE);
	gtk_combo_set_value_in_list(GTK_COMBO(title_convert_out_to_list), TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out_to_list, TRUE, TRUE, 0);

	gtk_widget_set_sensitive(title_convert_out_to, mpg123_cfg.convert_out);
	gtk_entry_set_text(GTK_ENTRY(title_convert_out_to), mpg123_cfg.convert_out_to);
	gtk_box_pack_start(GTK_BOX(title_convert_out_hbox), title_convert_out_to, FALSE, FALSE, 0);

	/* convert list */
	{
		GList *list = xmms_converter_charset_list();

		gtk_combo_set_popdown_strings(GTK_COMBO(title_convert_in_from_list), list);
		gtk_combo_set_popdown_strings(GTK_COMBO(title_convert_in_to_list), list);
		gtk_combo_set_popdown_strings(GTK_COMBO(title_convert_out_from_list), list);
		gtk_combo_set_popdown_strings(GTK_COMBO(title_convert_out_to_list), list);
		gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(title_convert_in_from_list)->entry), xmms_converter_get_title_by_name(mpg123_cfg.convert_in_from));
		gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(title_convert_in_to_list)->entry), xmms_converter_get_title_by_name(mpg123_cfg.convert_in_to));
		gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(title_convert_out_from_list)->entry), xmms_converter_get_title_by_name(mpg123_cfg.convert_out_from));
		gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(title_convert_out_to_list)->entry), xmms_converter_get_title_by_name(mpg123_cfg.convert_out_to));
		gtk_widget_set_sensitive(title_convert_in_from_list, mpg123_cfg.convert_in);
		gtk_widget_set_sensitive(title_convert_in_to_list, mpg123_cfg.convert_in);
		gtk_widget_set_sensitive(title_convert_out_from_list, mpg123_cfg.convert_out);
		gtk_widget_set_sensitive(title_convert_out_to_list, mpg123_cfg.convert_out);
	}
	/* convert list end */
