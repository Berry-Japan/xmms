...
static void init(void)
{
	...
	xmms_cfg_read_boolean(cfg, "MPG123", "convert_in", &mpg123_cfg.convert_in);
	if (!xmms_cfg_read_string(cfg, "MPG123", "convert_in_from", &mpg123_cfg.convert_in_from)) {
#ifdef KANJI
		mpg123_cfg.convert_in_from = g_strdup("SHIFT_JIS");
#else
	        mpg123_cfg.convert_in_from = g_strdup("noconv");
#endif
	}
	if (!xmms_cfg_read_string(cfg, "MPG123", "convert_in_to", &mpg123_cfg.convert_in_to)) {
#ifdef KANJI
		mpg123_cfg.convert_in_to = g_strdup("guess");
#else
		mpg123_cfg.convert_in_to = g_strdup("noconv");
#endif
	}
	xmms_cfg_read_boolean(cfg, "MPG123", "convert_out", &mpg123_cfg.convert_out);
	if (!xmms_cfg_read_string(cfg, "MPG123", "convert_out_from", &mpg123_cfg.convert_out_from)) {
#ifdef KANJI
		mpg123_cfg.convert_out_from = g_strdup("guess");
#else
		mpg123_cfg.convert_out_from = g_strdup("noconv");
#endif
	}
	if (!xmms_cfg_read_string(cfg, "MPG123", "convert_out_to", &mpg123_cfg.convert_out_to)){
#ifdef KANJI
		mpg123_cfg.convert_out_to = g_strdup("SHIFT_JIS");
#else
	        mpg123_cfg.convert_out_to = g_strdup("noconv");
#endif
	}
...
static char *mpg123_convertstr(char *str)
{
	int c, len, maxlen, err;
	gchar *d;
	...
	if ((len = xmms_converter_convert(str, &d, mpg123_cfg.convert_in_from, mpg123_cfg.convert_in_to, &err)) < 0 || d == NULL)
		return NULL;
	if (err != 0) {
		free(d);
		return str;
...
char *mpg123_format_song_title(struct id3v2tag_t *tag, char *filename)
{
	...
	if (tag)
	{
		if (mpg123_cfg.convert_in)
		{
			input->performer = mpg123_convertstr(tag->artist);
			input->album_name = mpg123_convertstr(tag->album);
			input->track_name = mpg123_convertstr(tag->title);
			input->genre = mpg123_convertstr(tag->genre);
			input->comment = mpg123_convertstr(tag->comment);
		}
		else
		{
