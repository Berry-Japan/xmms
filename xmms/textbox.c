...
static void textbox_draw_pixmap_scrolled(TextBox * tb);
static void textbox_handle_special_char(char, int *, int *);
...
static gint textbox_scroll(gpointer data)
{
	...
	if (!tb->tb_is_dragging)
	{
		...
		if (cfg.textbox_transparent)
			textbox_draw_pixmap_scrolled(tb);
		draw_widget(tb);
	}
	return TRUE;
}
...
static void textbox_draw_xfont_pixmap_scrolled(TextBox * tb)
{
	...
	gint length = strlen(pixmaptext);
	GdkColor *c, pattern;
	GdkGC *gc = tb->tb_widget.gc;
	GdkBitmap *mask = gdk_pixmap_new(mainwin->window,
					 tb->tb_pixmap_width, tb->tb_widget.height, 1);
...
static void textbox_draw_pixmap_scrolled(TextBox * tb)
{
	...
	skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_MAIN,
			 tb->tb_widget.x, tb->tb_widget.y,
			 tb->tb_offset, 0, tb->tb_widget.width, tb->tb_widget.height);
	w = tb->tb_pixmap_width - tb->tb_offset;
	if (w < tb->tb_widget.width)
		skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_MAIN,
				 tb->tb_widget.x + w, tb->tb_widget.y,
				 0, 0, tb->tb_widget.width, tb->tb_widget.height);
...
extern GdkPixmap *playlistwin_bg;

static void textbox_generate_pixmap(TextBox * tb)
{
	gint length, i, n, x, y, wl;
	gchar *pixmaptext;
	GdkGC *gc;
	...
	if (cfg.textbox_transparent) {
		if (tb->tb_widget.parent == playlistwin_bg) {
			if (cfg.playlist_shaded) {
				n = (tb->tb_widget.width - 40) / 25;
				skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_PLEDIT, 72 + tb->tb_widget.x, 42 + tb->tb_widget.y, 0, 0, 25 - tb->tb_widget.x, tb->tb_widget.height);
				for (i = 0; i < n; i++)
					skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_PLEDIT, 72, 57 + tb->tb_widget.y, 25 - tb->tb_widget.x + (i * 25), 0, 25, tb->tb_widget.height);
				skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_PLEDIT, 99, 42 + tb->tb_widget.y, tb->tb_widget.width - (40 - 25 + tb->tb_widget.x), 0, 40 - 25 + tb->tb_widget.x, tb->tb_widget.height);
			} else {
				skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_PLEDIT, 126 + 150 - (cfg.playlist_width - tb->tb_widget.x), 72 + 38 - (cfg.playlist_height - tb->tb_widget.y), 0, 0, tb->tb_widget.width, tb->tb_widget.height);
			}
		} else {
			skin_draw_pixmap(tb->tb_pixmap, gc, SKIN_MAIN, tb->tb_widget.x, tb->tb_widget.y, 0, 0, tb->tb_widget.width, tb->tb_widget.height);
		}
