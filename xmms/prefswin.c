...
static GtkWidget *prefswin_options_tt_entry;
...
void prefswin_apply_changes(void)
{
	...
	cfg.transparent_threshold = CLAMP(atoi(gtk_entry_get_text(GTK_ENTRY(prefswin_options_tt_entry))), 0, 99) << 8;
...
void create_prefs_window(void)
{
	...
	GtkWidget *options_tt, *options_tt_box, *options_tt_label;
	...
	options_pbs = prefswin_option_new_to_table(&cfg.pause_between_songs, GTK_TABLE(options_table), 1, 7);
	...
	options_sw = prefswin_option_new_to_table(&cfg.snap_windows, GTK_TABLE(options_table), 1, 8);
	...
	gtk_table_attach_defaults(GTK_TABLE(options_table), options_mouse_box, 1, 2, 9, 10);

	options_tt = prefswin_option_new_to_table(&cfg.textbox_transparent, GTK_TABLE(options_table), 0, 9);
	options_tt_box = gtk_hbox_new(FALSE, 5);
	options_tt_label = gtk_label_new(_("Transparent textbox: Threshold"));
	gtk_box_pack_start(GTK_BOX(options_tt_box), options_tt_label, FALSE, FALSE, 0);
	prefswin_options_tt_entry = gtk_entry_new_with_max_length(2);
	gtk_widget_set_usize(prefswin_options_tt_entry, 30, -1);
	gtk_box_pack_start(GTK_BOX(options_tt_box), prefswin_options_tt_entry, FALSE, FALSE, 0);
	gtk_container_add(GTK_CONTAINER(options_tt), options_tt_box);
	gtk_tooltips_set_tip(prefswin_tooltips, options_tt, _("Some skins have BMPs with an unclear background.  If you'd like to get clear transparency, adjust this value.  When we determine which pixel should be made transparent, we calculate the maximum RGB difference from the base pixel.  If the difference is smaller than this threshold, the pixel will be made transparent."), NULL);

	prefswin_option_new_with_label_to_table(&cfg.equalizer_transparent, _("Transparent equalizer"), GTK_TABLE(options_table), 0, 10);
	prefswin_option_new_with_label_to_table(&cfg.vis_transparent, _("Transparent visualizer"), GTK_TABLE(options_table), 0, 11);

	opt = prefswin_option_new_with_label_to_table(&cfg.use_pl_metadata,
						      _("Use meta-data in playlists"),
						      GTK_TABLE(options_table), 1, 10);
	gtk_tooltips_set_tip(prefswin_tooltips, opt,
...
void show_prefs_window(void)
{
	...
	sprintf(temp, "%u", cfg.transparent_threshold >> 8);
	gtk_entry_set_text(GTK_ENTRY(prefswin_options_tt_entry), temp);
