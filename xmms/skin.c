...
static int skin_diff_color(GdkColor *c1, GdkColor *c2)
{
	int diff, max;

	max = (c1->red > c2->red) ? c1->red - c2->red : c2->red - c1->red;
	diff = (c1->green > c2->green) ? c1->green - c2->green : c2->green - c1->green;
	if (max < diff)
		max = diff;
	diff = (c1->blue > c2->blue) ? c1->blue - c2->blue : c2->blue - c1->blue;
	if (max < diff)
		max = diff;
	return max;
}

static int create_mask(GdkWindow *window, GdkPixmap *pixmap, GdkBitmap *masks[], int w, int h, int xu, int yu, int tx, int ty)
{
	GdkGC *gc;
	GdkColor tc, c, pattern;
	int i, j, k, x, y;
	GdkImage *gi;
	...
	gi = gdk_image_get(pixmap, 0, 0, w * xu, h * yu);
	if (!gi) {
		fprintf(stderr, "Warning: gdk_image_get failed (%d,%d).\n", w * xu, h * yu);
		return 0;
	}
	tc.pixel = gdk_image_get_pixel(gi, tx, ty);
	skin_query_color(gdk_window_get_colormap(window), &tc);

	k = 0;
	for (i = 0; i < yu; i++) {
		for (j = 0; j < xu; j++) {
			if (!masks[k])
				masks[k] = gdk_pixmap_new(window, w, h, 1);
			gc = gdk_gc_new(masks[k]);
			pattern.pixel = 0;
			gdk_gc_set_foreground(gc, &pattern);
			gdk_draw_rectangle(masks[k], gc, TRUE, 0, 0, w, h);
			pattern.pixel = 1;
			gdk_gc_set_foreground(gc, &pattern);
			for (y = 0; y < h; y++) {
				for (x = 0; x < w; x++) {
					c.pixel = gdk_image_get_pixel(gi, j * w + x, i * h + y);
					skin_query_color(gdk_window_get_colormap(window), &c);
					if (skin_diff_color(&c, &tc) > cfg.transparent_threshold)
						gdk_draw_point(masks[k], gc, x, y);
				}
			}
			gdk_gc_destroy(gc);
			k++;
...
#define NUMBER_XUNITS 12
#define NUMBER_YUNITS 1
static int create_numbers_mask(GdkWindow *window, GdkPixmap *pixmap, GdkBitmap *masks[], gint h)
{
	return create_mask(window, pixmap, masks, NUMBER_WIDTH, h, NUMBER_XUNITS, NUMBER_YUNITS, 0, 0);
}

#define TEXT_WIDTH 5
#define TEXT_HEIGHT 6
#define TEXT_XUNITS 31
#define TEXT_YUNITS 3
static int create_text_mask(GdkWindow *window, GdkPixmap *pixmap, GdkBitmap *masks[], gint h)
{
	return create_mask(window, pixmap, masks, TEXT_WIDTH, TEXT_HEIGHT, TEXT_XUNITS, h / TEXT_HEIGHT, TEXT_WIDTH*30+3, 3);
}
...
void init_skins(void)
{
	...
	if (!create_numbers_mask(mainwin->window, skin->numbers.def_pixmap, skin->def_nums_mask, NUMBER_HEIGHT))
		fprintf(stderr, "create_numbers_mask(1) failed\n");
	if (!create_text_mask(mainwin->window, skin->text.def_pixmap, skin->def_text_mask, TEXT_HEIGHT * TEXT_YUNITS))
		fprintf(stderr, "create_text_mask(1) failed\n");
			     ...
			     const gchar * path, const gchar * file)
{
	...
	if (cfg.equalizer_transparent && !strcmp(file, "eqmain.bmp"))
	{
...
static void skin_numbers_generate_dash(SkinPixmap *numbers)
{
	...
	skin_draw_pixmap(pixmap, gc, SKIN_NUMBERS_NOMASK,
			 0, 0, 0, 0, 99, 13);
	skin_draw_pixmap(pixmap, gc, SKIN_NUMBERS_NOMASK,
			 90, 0, 99, 0, 9, 13);
	skin_draw_pixmap(pixmap, gc, SKIN_NUMBERS_NOMASK,
...
static void skin_load_pixmaps(const char *path)
{
	...
	else if (skin->numbers.current_width < 108)
	{
		skin_numbers_generate_dash(&skin->numbers);
	} 
	...
	if (!create_numbers_mask(mainwin->window, skin->numbers.pixmap ? : skin->numbers.def_pixmap, skin->mask_nums, skin->numbers.current_height))
		fprintf(stderr, "create_numbers_mask(2) failed\n");
	if (!create_text_mask(mainwin->window, skin->text.pixmap ? : skin->text.def_pixmap, skin->mask_text, skin->text.current_height))
		fprintf(stderr, "create_text_mask(2) failed\n");
...
{
	char *tmp = NULL, *tempdir, *unzip, *tar, *lha, *ending, *escaped;
	...
	if (!strcasecmp(ending, ".lzh"))
		tmp = g_strdup_printf("%s >/dev/null ew=%s \"%s\"", lha, tempdir, escaped);
		      ...
		      gint width, gint height)
{
	...
	if (pixmap->pixmap != NULL)
	{
		...
		/* XXX: dirty... */
		num_mask = skin->mask_nums[xsrc / NUMBER_WIDTH];
		text_mask = skin->mask_text[((int)(ysrc / TEXT_HEIGHT)) * TEXT_XUNITS + (xsrc / TEXT_WIDTH)];
	}
	else
	{
		tmp = pixmap->def_pixmap;
		/* XXX: dirty... */
		num_mask = skin->def_nums_mask[xsrc / NUMBER_WIDTH];
		text_mask = skin->def_text_mask[((int)(ysrc / TEXT_HEIGHT)) * TEXT_XUNITS + (xsrc / TEXT_WIDTH)];
	}

	if (cfg.textbox_transparent) {
		GdkBitmap *mask = NULL;
		GdkGC *maskgc = gdk_gc_new(drawable);

		gdk_gc_copy(maskgc, gc);
		...
		if (mask) {
			gdk_gc_set_clip_mask(maskgc, mask);
			gdk_gc_set_clip_origin(maskgc, xdest, ydest);
		}
		gdk_draw_pixmap(drawable, maskgc, tmp, xsrc, ysrc, xdest, ydest, width, height);
		gdk_gc_unref(maskgc);
	} else {
